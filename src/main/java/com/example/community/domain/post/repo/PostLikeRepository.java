...
public interface PostLikeRepository extends JpaRepository<PostLike, Long> {
    ...
    @Query(value = "select new com.example.community.domain.post.vo.LikeVo(p.post.id, count(p.id)) from PostLike p where p.post.id in :postIdList group by p.post.id")
